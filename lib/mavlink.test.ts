...
import { uint8_t, uint16_t, x25crc, uint64_t } from "mavlink-mappings";
...
describe("test parser MavLinkProtocolV2", () => {
  it("parses untrimmed buffer", () => {
    ...
    const msg = mavlink.data(
      Buffer.from([0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x00, 0x00]),
      SingleUint64FieldMessage
    );
    expect(msg.uint64field).toBe(
      Buffer.from([
  ...
  it("parses untrimmed two field buffer", () => {
    const mavlink = new MavLinkProtocolV2();
    const msg = mavlink.data(
      Buffer.from([0x09, 0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x01, 0x00]),
      TwoFieldMessage
    );
    expect(msg.uint8Field).toBe(Buffer.from([0x09]).readUInt8());
    expect(msg.uint64field).toBe(
      Buffer.from([
        0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x01, 0x00,
      ]).readBigUInt64LE()
    );
  });
  it("parses trimmed two field buffer", () => {
    const mavlink = new MavLinkProtocolV2();
    const msg = mavlink.data(
      Buffer.from([0x09, 0xde, 0xad, 0xbe, 0xef]),
      TwoFieldMessage
    );
    expect(msg.uint8Field).toBe(Buffer.from([0x09]).readUInt8());
    expect(msg.uint64field).toBe(
